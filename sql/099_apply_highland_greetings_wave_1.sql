-- 099_apply_highland_greetings_wave_1.sql
-- Purpose:
-- Apply the validated first eastern highland greetings/courtesy wave.

begin;

create temporary table _target_highland_greetings_w1_batches (
  batch_label text primary key
) on commit drop;

insert into _target_highland_greetings_w1_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_highland_greetings_w1_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_highland_greetings_w1_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_EBU_GREETINGS_HIGHLAND_W1_2026_02_27_A',
--   'entry_import_staging:BATCH_MER_GREETINGS_HIGHLAND_W1_2026_02_27_A',
--   'entry_import_staging:BATCH_GUZ_GREETINGS_HIGHLAND_W1_2026_02_27_A',
--   'entry_import_staging:BATCH_KUR_GREETINGS_HIGHLAND_W1_2026_02_27_A'
-- )
