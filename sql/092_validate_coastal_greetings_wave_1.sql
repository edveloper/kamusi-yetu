-- 092_validate_coastal_greetings_wave_1.sql
-- Purpose:
-- Validate the first remaining coastal greetings/courtesy wave.

begin;

with target_batches(batch_label) as (
  values
    ('BATCH_NYF_GREETINGS_COASTAL_W1_2026_02_27_A'::text),
    ('BATCH_COH_GREETINGS_COASTAL_W1_2026_02_27_A'::text)
),
base as (
  select
    s.id,
