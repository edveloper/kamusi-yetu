-- 056_apply_kalenjin_subgroup_wave1_kipsigis_nandi.sql
-- Purpose:
-- Apply the validated first Kalenjin subgroup starter wave for Kipsigis and Nandi.

begin;

create temporary table _target_kalenjin_wave1_batches (
  batch_label text primary key
) on commit drop;

insert into _target_kalenjin_wave1_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_kalenjin_wave1_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_kalenjin_wave1_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_SGC_STARTER_2026_02_26_A',
--   'entry_import_staging:BATCH_NIQ_STARTER_2026_02_26_A'
-- )
