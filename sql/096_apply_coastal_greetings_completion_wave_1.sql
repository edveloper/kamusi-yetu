-- 096_apply_coastal_greetings_completion_wave_1.sql
-- Purpose:
-- Apply the validated first coastal greetings completion wave.

begin;

create temporary table _target_coastal_greetings_completion_w1_batches (
  batch_label text primary key
) on commit drop;

insert into _target_coastal_greetings_completion_w1_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_coastal_greetings_completion_w1_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_coastal_greetings_completion_w1_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_COH_GREETINGS_COMPLETION_W1_2026_02_27_A',
--   'entry_import_staging:BATCH_NYF_GREETINGS_COMPLETION_W1_2026_02_27_A'
-- )
