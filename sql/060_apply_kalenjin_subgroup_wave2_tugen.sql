-- 060_apply_kalenjin_subgroup_wave2_tugen.sql
-- Purpose:
-- Apply the validated second Kalenjin subgroup starter wave for Tugen.

begin;

create temporary table _target_kalenjin_wave2_batches (
  batch_label text primary key
) on commit drop;

insert into _target_kalenjin_wave2_batches (batch_label)
values
  ('BATCH_TUY_STARTER_2026_02_27_A');
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_kalenjin_wave2_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_kalenjin_wave2_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref = 'entry_import_staging:BATCH_TUY_STARTER_2026_02_27_A'
