-- 075_apply_greetings_courtesy_wave_d.sql
-- Purpose:
-- Apply the validated fourth greetings/courtesy wave.

begin;

create temporary table _target_greetings_wave_d_batches (
  batch_label text primary key
) on commit drop;

insert into _target_greetings_wave_d_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_greetings_wave_d_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_greetings_wave_d_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_BXK_GREETINGS_WAVE_D_2026_02_27_A',
--   'entry_import_staging:BATCH_LTO_GREETINGS_WAVE_D_2026_02_27_A',
--   'entry_import_staging:BATCH_SXB_GREETINGS_WAVE_D_2026_02_27_A',
--   'entry_import_staging:BATCH_ORC_GREETINGS_WAVE_D_2026_02_27_A',
--   'entry_import_staging:BATCH_MAS_GREETINGS_WAVE_D_2026_02_27_A',
--   'entry_import_staging:BATCH_SHENG_GREETINGS_WAVE_D_2026_02_27_A'
-- )
