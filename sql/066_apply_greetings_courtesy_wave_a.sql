-- 066_apply_greetings_courtesy_wave_a.sql
-- Purpose:
-- Apply the validated first broad greetings/courtesy wave.

begin;

create temporary table _target_greetings_wave_a_batches (
  batch_label text primary key
) on commit drop;

insert into _target_greetings_wave_a_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_greetings_wave_a_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_greetings_wave_a_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_KI_GREETINGS_WAVE_A_2026_02_27_A',
--   'entry_import_staging:BATCH_LUO_GREETINGS_WAVE_A_2026_02_27_A',
--   'entry_import_staging:BATCH_KAM_GREETINGS_WAVE_A_2026_02_27_A',
--   'entry_import_staging:BATCH_LUY_GREETINGS_WAVE_A_2026_02_27_A',
--   'entry_import_staging:BATCH_KAL_GREETINGS_WAVE_A_2026_02_27_A'
-- )
