-- 098_validate_highland_greetings_wave_1.sql
-- Purpose:
-- Validate the first eastern highland greetings/courtesy wave.

begin;

with target_batches(batch_label) as (
  values
    ('BATCH_EBU_GREETINGS_HIGHLAND_W1_2026_02_27_A'::text),
    ('BATCH_MER_GREETINGS_HIGHLAND_W1_2026_02_27_A'::text),
    ('BATCH_GUZ_GREETINGS_HIGHLAND_W1_2026_02_27_A'::text),
    ('BATCH_KUR_GREETINGS_HIGHLAND_W1_2026_02_27_A'::text)
),
base as (
  select
    s.id,
...
-- select batch_label, status, count(*) as rows
