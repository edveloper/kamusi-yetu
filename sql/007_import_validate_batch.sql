-- 007_import_validate_batch.sql
-- Purpose:
-- Validate one batch in entry_import_staging and mark each row as:
-- - validated (no errors)
-- - rejected (one or more errors)
--
-- IMPORTANT:
...
with params as (
  select 'BATCH_LABEL_HERE'::text as batch_label
),
base as (
  select
    s.id,
...
-- with params as (select 'BATCH_LABEL_HERE'::text as batch_label)
-- select
--   status,
--   count(*) as rows
-- from public.entry_import_staging
-- where batch_label = (select batch_label from params)
-- group by status
-- order by status;
