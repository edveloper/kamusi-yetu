-- 025_apply_active_language_phrase_batches.sql
-- Purpose:
-- Apply all five validated active-language phrase batches in one run,
-- while still creating one import job record per batch.
--
-- Batches covered:
...
create temporary table _target_phrase_batches (
  batch_label text primary key
) on commit drop;

insert into _target_phrase_batches (batch_label)
values
  ('BATCH_KI_PHRASES_2026_02_25_A'),
  ('BATCH_LUO_PHRASES_2026_02_25_A'),
  ('BATCH_KAM_PHRASES_2026_02_25_A'),
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_phrase_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_phrase_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_KI_PHRASES_2026_02_25_A',
--   'entry_import_staging:BATCH_LUO_PHRASES_2026_02_25_A',
--   'entry_import_staging:BATCH_KAM_PHRASES_2026_02_25_A',
--   'entry_import_staging:BATCH_LUY_PHRASES_2026_02_25_A',
--   'entry_import_staging:BATCH_KAL_PHRASES_2026_02_25_A'
-- )
