-- 107_validate_remaining_zero_greetings_completion_wave_1.sql
begin;

with target_batches(batch_label) as (
  values
    ('BATCH_DIG_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_DUG_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_POK_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_RRA_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_REL_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_DAV_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_TVS_GREETINGS_COMPLETION_W1_2026_02_27_A'::text),
    ('BATCH_TRK_GREETINGS_COMPLETION_W1_2026_02_27_A'::text)
),
base as (
  select
    s.id,
  ...
  left join public.languages l on lower(l.code) = lower(s.language_code)
),
dupes_in_batch as (
...
evaluated as (
  select
    ...
    array_remove(array[
      ...
      case when b.resolved_code = 'en' and nullif(trim(coalesce(b.swahili_translation, '')), '') is null then 'EN_REQUIRES_SWAHILI_TRANSLATION' end,
      case when b.resolved_code = 'sw' and nullif(trim(coalesce(b.english_translation, '')), '') is null then 'SW_REQUIRES_ENGLISH_TRANSLATION' end,
      case when b.resolved_code not in ('en', 'sw')
        and nullif(trim(coalesce(b.english_translation, '')), '') is null
        and nullif(trim(coalesce(b.swahili_translation, '')), '') is null then 'MISSING_BRIDGE_TRANSLATION' end,
