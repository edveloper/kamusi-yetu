-- 078_apply_greetings_courtesy_wave_e.sql
-- Purpose:
-- Apply the validated fifth greetings/courtesy wave.

begin;

create temporary table _target_greetings_wave_e_batches (
  batch_label text primary key
) on commit drop;

insert into _target_greetings_wave_e_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_greetings_wave_e_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_greetings_wave_e_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_MAS_GREETINGS_WAVE_E_2026_02_27_A',
--   'entry_import_staging:BATCH_ORC_GREETINGS_WAVE_E_2026_02_27_A',
--   'entry_import_staging:BATCH_SGC_GREETINGS_WAVE_E_2026_02_27_A',
--   'entry_import_staging:BATCH_NIQ_GREETINGS_WAVE_E_2026_02_27_A',
--   'entry_import_staging:BATCH_SPY_GREETINGS_WAVE_E_2026_02_27_A',
--   'entry_import_staging:BATCH_SHENG_GREETINGS_WAVE_E_2026_02_27_A'
-- )
