-- 052_apply_luhya_subgroup_wave2_bukusu_logooli.sql
-- Purpose:
-- Apply the validated second Luhya subgroup starter wave for Bukusu and Logooli.

begin;

create temporary table _target_luhya_wave2_batches (
  batch_label text primary key
) on commit drop;

insert into _target_luhya_wave2_batches (batch_label)
values
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_luhya_wave2_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_luhya_wave2_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref in (
--   'entry_import_staging:BATCH_BXK_STARTER_2026_02_26_A',
--   'entry_import_staging:BATCH_RAG_STARTER_2026_02_26_A'
-- )
