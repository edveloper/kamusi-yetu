-- 084_apply_samburu_greetings_pass.sql
-- Purpose:
-- Apply the validated dedicated Samburu greetings/courtesy pass.

begin;

create temporary table _target_samburu_greetings_batches (
  batch_label text primary key
) on commit drop;

insert into _target_samburu_greetings_batches (batch_label)
values
  ('BATCH_SAQ_GREETINGS_2026_02_27_A');
...
with inserted as (
  ...
  select
    ...
    now()
  from _target_samburu_greetings_batches tb
  where exists (
    select 1
...
select
  ...
  s.created_by
from public.entry_import_staging s
join _target_samburu_greetings_batches tb on tb.batch_label = s.batch_label
...
-- where source_ref = 'entry_import_staging:BATCH_SAQ_GREETINGS_2026_02_27_A'
